...
#desc Return the text parsed by [method parse] as long as the function is instructed to keep it.
func get_parsed_text() -> String:
	pass;

#desc Attempts to parse the [param json_text] provided.
#desc Returns an [enum Error]. If the parse was successful, it returns [constant OK] and the result can be retrieved using [member data]. If unsuccessful, use [method get_error_line] and [method get_error_message] for identifying the source of the failure.
#desc Non-static variant of [method parse_string], if you want custom error handling.
#desc The optional [param keep_text] argument instructs the parser to keep a copy of the original text. This text can be obtained later by using the [method get_parsed_text] function and is used when saving the resource (instead of generating new text from [member data]).
