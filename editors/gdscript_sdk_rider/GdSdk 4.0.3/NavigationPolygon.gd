...
#desc $NavigationRegion2D.navigation_polygon = polygon
#desc [/gdscript]
#desc [csharp]
...
#desc GetNode<NavigationRegion2D>("NavigationRegion2D").NavigationPolygon = polygon;
#desc [/csharp]
#desc [/codeblocks]
...
#desc Returns the [NavigationMesh] resulting from this navigation polygon. This navigation mesh can be used to update the navigation mesh of a region with the [method NavigationServer3D.region_set_navigation_mesh] API directly (as 2D uses the 3D server behind the scene).
