...
#desc 
#desc [Tool]
#desc public partial class CustomParser : EditorTranslationParserPlugin
#desc {
#desc public override void _ParseFile(string path, Godot.Collections.Array<string> msgids, Godot.Collections.Array<Godot.Collections.Array> msgidsContextPlural)
#desc {
#desc using var file = FileAccess.Open(path, FileAccess.ModeFlags.Read);
#desc string text = file.GetAsText();
#desc string[] splitStrs = text.Split(",", allowEmpty: false);
#desc foreach (string s in splitStrs)
#desc {
#desc msgids.Add(s);
