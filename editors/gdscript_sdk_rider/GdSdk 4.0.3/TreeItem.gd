...
#desc Adds a button with [Texture2D] [param button] at column [param column]. The [param id] is used to identify the button in the according [signal Tree.button_clicked] signal and can be different from the buttons index. If not specified, the next available index is used, which may be retrieved by calling [method get_button_count] immediately before this method. Optionally, the button can be [param disabled] and have a [param tooltip_text].
...
#desc The new item will be inserted as position [param index] (the default value [code]-1[/code] means the last position), or it will be the last child if [param index] is higher than the child count.
...
#desc Removes the button at index [param button_index] in column [param column].
func erase_button(column: int, button_index: int) -> void:
	pass;

#desc Returns the [Texture2D] of the button at index [param button_index] in column [param column].
...
#desc Returns the ID for the button at index [param button_index] in column [param column].
func get_button_id(column: int, button_index: int) -> int:
	pass;

#desc Returns the tooltip text for the button at index [param button_index] in column [param column].
...
#desc Returns a child item by its [param index] (see [method get_child_count]). This method is often used for iterating all children of an item.
...
#desc Returns [code]true[/code] if the button at index [param button_index] for the given [param column] is disabled.
...
#desc Sets the given column's button [Texture2D] at index [param button_index] to [param button].
func set_button(column: int, button_index: int, button: Texture2D) -> void:
	pass;

#desc Sets the given column's button color at index [param button_index] to [param color].
func set_button_color(column: int, button_index: int, color: Color) -> void:
	pass;

#desc If [code]true[/code], disables the button at index [param button_index] in the given [param column].
