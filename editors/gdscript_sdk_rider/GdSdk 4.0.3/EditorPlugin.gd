...
#tutorial [Editor plugins documentation index] https://docs.godotengine.org/en/stable/tutorials/plugins/editor/index.html
class_name EditorPlugin

#desc Emitted when user changes the workspace ([b]2D[/b], [b]3D[/b], [b]Script[/b], [b]AssetLib[/b]). Also works with custom screens defined by plugins.
signal main_screen_changed(screen_name: String)
#desc Emitted when any project setting has changed.
signal project_settings_changed
#desc Emitted when the given [param resource] was saved on disc.
signal resource_saved(resource: Resource)
#desc Emitted when the scene is changed in the editor. The argument will return the root node of the scene that has just become active. If this scene is new and empty, the argument will be [code]null[/code].
signal scene_changed(scene_root: Node)
#desc Emitted when user closes a scene. The argument is file path to a closed scene.
signal scene_closed(filepath: String)
#desc Main editor toolbar, next to play buttons.
#desc The toolbar that appears when 3D editor is active.
#desc Left sidebar of the 3D editor.
#desc Right sidebar of the 3D editor.
#desc Bottom panel of the 3D editor.
#desc The toolbar that appears when 2D editor is active.
#desc Left sidebar of the 2D editor.
#desc Right sidebar of the 2D editor.
#desc Bottom panel of the 2D editor.
#desc Bottom section of the inspector.
#desc Tab of Project Settings dialog, to the left of other tabs.
#desc Tab of Project Settings dialog, to the right of other tabs.
#desc Dock slot, left side, upper-left (empty in default layout).
#desc Dock slot, left side, bottom-left (empty in default layout).
#desc Dock slot, left side, upper-right (in default layout includes Scene and Import docks).
#desc Dock slot, left side, bottom-right (in default layout includes FileSystem dock).
#desc Dock slot, right side, upper-left (empty in default layout).
#desc Dock slot, right side, bottom-left (empty in default layout).
#desc Dock slot, right side, upper-right (in default layout includes Inspector, Node and History docks).
#desc Dock slot, right side, bottom-right (empty in default layout).
...
#desc [param object] can be [code]null[/code] if the plugin was editing an object, but there is no longer any selected object handled by this plugin. It can be used to cleanup editing state.
...
#desc [/gdscript]
#desc [csharp]
#desc public override void _Forward3DDrawOverViewport(Control viewportControl)
#desc {
#desc // Draw a circle at cursor position.
#desc viewportControl.DrawCircle(viewportControl.GetLocalMousePosition(), 64, Colors.White);
#desc }
#desc 
#desc public override EditorPlugin.AfterGuiInput _Forward3DGuiInput(Camera3D viewportCamera, InputEvent @event)
#desc {
#desc if (@event is InputEventMouseMotion)
#desc {
#desc // Redraw viewport when cursor is moved.
#desc UpdateOverlays();
#desc return EditorPlugin.AfterGuiInput.Stop;
#desc }
#desc return EditorPlugin.AfterGuiInput.Pass;
#desc }
#desc [/csharp]
#desc [/codeblocks]
...
#desc Called when there is a root node in the current edited scene, [method _handles] is implemented, and an [InputEvent] happens in the 3D viewport. The return value decides whether the [InputEvent] is consumed or forwarded to other [EditorPlugin]s. See [enum AfterGUIInput] for options.
#desc [b]Example:[/b]
#desc [codeblocks]
#desc [gdscript]
#desc # Prevents the InputEvent from reaching other Editor classes.
...
#desc [/csharp]
#desc [/codeblocks]
#desc Must [code]return EditorPlugin.AFTER_GUI_INPUT_PASS[/code] in order to forward the [InputEvent] to other Editor classes.
#desc [b]Example:[/b]
#desc [codeblocks]
#desc [gdscript]
...
#desc public override EditorPlugin.AfterGuiInput _Forward3DGuiInput(Camera3D camera, InputEvent @event)
#desc {
#desc return @event is InputEventMouseMotion ? EditorPlugin.AfterGuiInput.Stop : EditorPlugin.AfterGuiInput.Pass;
#desc }
#desc [/csharp]
#desc [/codeblocks]
...
#desc [/gdscript]
#desc [csharp]
#desc public override void _ForwardCanvasDrawOverViewport(Control viewportControl)
#desc {
#desc // Draw a circle at cursor position.
#desc viewportControl.DrawCircle(viewportControl.GetLocalMousePosition(), 64, Colors.White);
#desc }
#desc 
#desc public override bool _ForwardCanvasGuiInput(InputEvent @event)
#desc {
#desc if (@event is InputEventMouseMotion)
#desc {
#desc // Redraw viewport when cursor is moved.
...
#desc Called when there is a root node in the current edited scene, [method _handles] is implemented and an [InputEvent] happens in the 2D viewport. Intercepts the [InputEvent], if [code]return true[/code] [EditorPlugin] consumes the [param event], otherwise forwards [param event] to other Editor classes.
#desc [b]Example:[/b]
#desc [codeblocks]
#desc [gdscript]
#desc # Prevents the InputEvent from reaching other Editor classes.
...
#desc [/csharp]
#desc [/codeblocks]
#desc Must [code]return false[/code] in order to forward the [InputEvent] to other Editor classes.
#desc [b]Example:[/b]
#desc [codeblocks]
#desc [gdscript]
#desc # Consumes InputEventMouseMotion and forwards other InputEvent types.
...
#desc [/gdscript]
#desc [csharp]
#desc public override Texture2D _GetPluginIcon()
#desc {
#desc // You can use a custom icon:
...
#desc [b]Note:[/b] The base type is the base engine class which this type's class hierarchy inherits, not any custom type parent classes.
...
func add_debugger_plugin(script: EditorDebuggerPlugin) -> void:
	pass;
...
#desc See [method add_inspector_plugin] for an example of how to register a plugin.
func add_import_plugin(importer: EditorImportPlugin, first_priority: bool = false) -> void:
	pass;
...
#desc Registers a new [EditorResourceConversionPlugin]. Resource conversion plugins are used to add custom resource converters to the editor inspector.
#desc See [EditorResourceConversionPlugin] for an example of how to create a resource conversion plugin.
func add_resource_conversion_plugin(plugin: EditorResourceConversionPlugin) -> void:
	pass;

#desc Registers a new [EditorSceneFormatImporter]. Scene importers are used to import custom 3D asset formats as scenes.
#desc If [param first_priority] is [code]true[/code], the new import plugin is inserted first in the list and takes precedence over pre-existing plugins.
func add_scene_format_importer_plugin(scene_format_importer: EditorSceneFormatImporter, first_priority: bool = false) -> void:
	pass;
...
#desc If [param first_priority] is [code]true[/code], the new import plugin is inserted first in the list and takes precedence over pre-existing plugins.
func add_scene_post_import_plugin(scene_import_plugin: EditorScenePostImportPlugin, first_priority: bool = false) -> void:
	pass;
...
func remove_debugger_plugin(script: EditorDebuggerPlugin) -> void:
	pass;
...
#desc Removes a resource conversion plugin registered by [method add_resource_conversion_plugin].
func remove_resource_conversion_plugin(plugin: EditorResourceConversionPlugin) -> void:
	pass;
...
#desc Removes a callback previously added by [method add_undo_redo_inspector_hook_callback].
