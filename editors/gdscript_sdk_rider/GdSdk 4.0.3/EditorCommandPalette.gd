...
#desc Command key names use slash delimiters to distinguish sections, for example: [code]"example/command1"[/code] then [code]example[/code] will be the section name.
#desc [codeblocks]
#desc [gdscript]
...
#desc Callable commandCallable = new Callable(this, MethodName.ExternalCommand);
