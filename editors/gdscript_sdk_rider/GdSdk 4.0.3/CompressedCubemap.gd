...
#brief 6-sided texture typically used in 3D rendering, optionally compressed.
#desc A cubemap that is loaded from a [code].ccube[/code] file. This file format is internal to Godot; it is created by importing other image formats with the import system. [CompressedCubemap] can use one of 4 compresson methods:
...
#desc See [Cubemap] for a general description of cubemaps.
class_name CompressedCubemap
